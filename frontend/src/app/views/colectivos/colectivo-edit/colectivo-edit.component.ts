...
export class ColectivoEditComponent implements OnInit {
  ...
  patenteIC = new UntypedFormControl('',[Validators.required, Validators.pattern('^[A-Z]{2}[0-9]{3}[A-Z]{2}$|^[A-Z]{3}[0-9]{3}$')] );
  marcaIC = new UntypedFormControl('', Validators.required );
  modeloIC = new UntypedFormControl('');
  anioIC = new UntypedFormControl('', Validators.pattern('^[12]{1}[09]{1}[0-9]{2}$') ); // 19xx o 20xx
  capacidadIC = new UntypedFormControl('', Validators.pattern('^[1-9][0-9]?$') ); // numero entre 1 y 100
  compraIC = new UntypedFormControl('');
